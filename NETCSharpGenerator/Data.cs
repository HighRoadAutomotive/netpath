...
namespace NETPath.Generators.NET.CS
{
	internal static class DataGenerator
	{
		...
		private static string GenerateElementDCMProxyCode45(DataElement o, ref int ProtoCount, bool IsOverride = false)
		{
			...
			if (o.DataType.TypeMode == DataTypeMode.Collection)
			{
				code.AppendLine(string.Format("\t\tpublic static readonly DeltaProperty<DeltaList<{2}>> {0}Property = DeltaProperty<DeltaList<{2}>>.RegisterList<{2}>(\"{1}\", typeof({1}), (s, o, n) => {{ var t = s as {1}; if (t == null) return;", o.HasClientType ? o.ClientName : o.DataName, o.Owner.HasClientType ? o.Owner.ClientType.Name : o.Owner.Name, DataTypeGenerator.GenerateType(o.DataType.CollectionGenericType)));
			...
			{
				code.AppendLine(string.Format("\t\tpublic static readonly DeltaProperty<{3}> {1}Property = DeltaProperty<{3}>.Register(\"{1}\", typeof({2}), default({3}), (s, o, n) => {{ var t = s as {2}; if (t == null) return; t.{1}PropertyChanged(o, n); }}, (s, o, n) => {{ var t = s as {2}; var nv = n as {0}; if (t == null || nv == null) return; var z = new {5}[nv.Length]; for (int i = 0; i < nv.Length; i++) z[i] = nv[i]; if (t.XAMLObject != null) t.XAMLObject.{4} = z; {6}}}, null, {7}, {8});", DataTypeGenerator.GenerateType(GetPreferredDTOType(o.DataType, o.DREEnabled)), o.HasClientType ? o.ClientName : o.DataName, o.Owner.HasClientType ? o.Owner.ClientType.Name : o.Owner.Name, DataTypeGenerator.GenerateType(o.DataType), o.XAMLName, DataTypeGenerator.GenerateType(GetPreferredXAMLType(o.DataType.CollectionGenericType, o.DREEnabled)), !o.DREPrimaryKey ? GenerateImmediateDREValueCallbacks(o.Owner.DataRevisionServiceNames, o.Owner.Name, o.DataName, false) : "", o.DREUpdateMode != DataUpdateMode.None && o.DREEnabled && !o.IsReadOnly && !o.DataType.IsCollectionType ? string.Format("{0}.{1}Property", o.Owner.XAMLType.Name, o.XAMLName) : "null", o.DREUpdateMode != DataUpdateMode.None && o.DREEnabled && o.IsReadOnly && !o.DataType.IsCollectionType ? string.Format("{0}.{1}PropertyKey", o.Owner.XAMLType.Name, o.XAMLName) : "null"));
				code.AppendLine(string.Format("\t\tpartial void {0}PropertyChanged({1} OldValue, {1} NewValue);", o.HasClientType ? o.ClientName : o.DataName, DataTypeGenerator.GenerateType(GetPreferredDTOType(o.DataType, o.Owner.CMDEnabled))));
			}
			else
			{
				code.AppendLine(string.Format("\t\tpublic static readonly DeltaProperty<{2}> {0}Property = DeltaProperty<{2}>.Register(\"{0}\", typeof({1}), default({2}), (s, o, n) => {{ var t = s as {1}; if (t == null) return; t.{0}PropertyChanged(o, n); }}, {3}, null, {4}, {5});", o.HasClientType ? o.ClientName : o.DataName, o.Owner.HasClientType ? o.Owner.ClientType.Name : o.Owner.Name, DataTypeGenerator.GenerateType(o.DataType), o.DREEnabled && !o.DREPrimaryKey && o.DREUpdateMode == DataUpdateMode.Immediate ? string.Format("(s, o, n) => {{ var t = s as {1}; if (t == null) return; {0}}}", GenerateImmediateDREValueCallbacks(o.Owner.DataRevisionServiceNames, o.Owner.Name, o.DataName, false), o.Owner.HasClientType ? o.Owner.ClientType.Name : o.Owner.Name) : "null", o.DREUpdateMode != DataUpdateMode.None && o.DREEnabled && !o.IsReadOnly && !o.DataType.IsCollectionType ? string.Format("{0}.{1}Property", o.Owner.XAMLType.Name, o.XAMLName) : "null", o.DREUpdateMode != DataUpdateMode.None && o.DREEnabled && o.IsReadOnly && !o.DataType.IsCollectionType ? string.Format("{0}.{1}PropertyKey", o.Owner.XAMLType.Name, o.XAMLName) : "null"));
				code.AppendLine(string.Format("\t\tpartial void {0}PropertyChanged({1} OldValue, {1} NewValue);", o.HasClientType ? o.ClientName : o.DataName, DataTypeGenerator.GenerateType(GetPreferredDTOType(o.DataType, o.Owner.CMDEnabled))));
			}
			return code.ToString();
		}
