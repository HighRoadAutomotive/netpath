...
namespace WCFArchitect.Compiler
{
	...
	internal class ProjectOptions
	{
		...
		public bool PlatformX86 { get; set; }
		public bool PlatformX64 { get; set; }
		public bool PlatformItanium { get; set; }
		public bool PlatformAnyCPU { get; set; }
		public bool ConfigurationDebug { get; set; }
		public bool ConfigurationRelease { get; set; }
	...
	internal class Compiler
	{
		...
		private bool IsPlatformX86(string ProjectName) { ProjectOptions po = null; Globals.CompilerManager.Options.TryGetValue(ProjectName, out po); return po.PlatformX86; }
		private bool IsPlatformX64(string ProjectName) { ProjectOptions po = null; Globals.CompilerManager.Options.TryGetValue(ProjectName, out po); return po.PlatformX64; } 
		private bool IsPlatformItanium(string ProjectName) { ProjectOptions po = null; Globals.CompilerManager.Options.TryGetValue(ProjectName, out po); return po.PlatformItanium; }
		private bool IsPlatformAnyCPU(string ProjectName) { ProjectOptions po = null; Globals.CompilerManager.Options.TryGetValue(ProjectName, out po); return po.PlatformAnyCPU; }
		private bool IsConfigurationDebug(string ProjectName) { ProjectOptions po = null; Globals.CompilerManager.Options.TryGetValue(ProjectName, out po); return po.ConfigurationDebug; }
		private bool IsConfigurationRelease(string ProjectName) { ProjectOptions po = null; Globals.CompilerManager.Options.TryGetValue(ProjectName, out po); return po.ConfigurationRelease; }
		...
		private bool CompileServer30(string ProjectName, List<CompilerOutputFile> DependencyAssemblies, List<CompilerOutputFile> OutputFiles)
		{
			...
			if (IsConfigurationDebug(ProjectName) == true)
			{
				string BaseOptions = " /debug:full /define:DEBUG;TRACE";

				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Debug);
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						Parameters.ReferencedAssemblies.Clear();
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						Parameters.ReferencedAssemblies.Clear();
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						Parameters.ReferencedAssemblies.Clear();
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
			...
			if (IsConfigurationRelease(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
			...
			if (IsConfigurationDebug(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Debug);
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
			...
			if (IsConfigurationRelease(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET30 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET30, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET30) + COF.FileName + ".dll";
			...
			if (IsConfigurationDebug(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
			...
			if (IsConfigurationRelease(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
			...
			if (IsConfigurationDebug(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
			...
			if (IsConfigurationRelease(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35) + COF.FileName + ".dll";
			...
			if (IsConfigurationDebug(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsServerFile == true);
						foreach (var D in DL)
						...
						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
			...
			if (IsConfigurationRelease(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
			...
			if (IsConfigurationDebug(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
			...
			if (IsConfigurationRelease(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET35Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET35Client, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET35Client) + COF.FileName + ".dll";
			...
			if (IsConfigurationDebug(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
			...
			if (IsConfigurationRelease(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
			...
			if (IsConfigurationDebug(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
			...
			if (IsConfigurationRelease(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40 && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40) + COF.FileName + ".dll";
			...
			if (IsConfigurationDebug(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsServerFile == true);
						foreach (var D in DL)
						...
						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
			...
			if (IsConfigurationRelease(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsServerFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ServerAssemblyName, true, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Server, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
			...
			if (IsConfigurationDebug(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Debug && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Debug);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
			...
			if (IsConfigurationRelease(ProjectName) == true)
			{
				...
				if (IsPlatformAnyCPU(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.AnyCPU && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.AnyCPU, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:anycpu ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX86(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x86 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.x86, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x86 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformX64(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.x64 && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.x64, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:x64 ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
				...
				if (IsPlatformItanium(ProjectName) == true)
				{
					try
					{
						...
						foreach (Projects.Reference R in RL)
							Parameters.ReferencedAssemblies.Add(R.Path);
						var DL = DependencyAssemblies.Where(a => a.Platform == Projects.ProjectOutputPlatform.Itanium && a.Configuration == Projects.ProjectOutputConfiguration.Release && a.Framework == Projects.ProjectOutputFramework.NET40Client && a.IsClientFile == true);
						foreach (var D in DL)
							Parameters.ReferencedAssemblies.Add(GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + D.FileName + ".dll");

						CompilerOutputFile COF = new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client), ClientAssemblyName, false, Projects.ProjectOutputFramework.NET40Client, Projects.ProjectOutputPlatform.Itanium, Projects.ProjectOutputConfiguration.Release);
						AddOutputLine("Building '" + COF.FileName + ".dll' ... ", CompilerMessageStage.Client, false);
						Parameters.CompilerOptions = BaseOptions + " /platform:Itanium ";
						Parameters.OutputAssembly = GetBuildOutputPath(ProjectName, Projects.ProjectOutputFramework.NET40Client) + COF.FileName + ".dll";
		...
		private void Output30(string ProjectName, List<CompilerOutputFile> OutputFiles)
		{
			...
			foreach (Projects.ProjectOutputPath OP in SOPL)
			{
				if (OP.GenerateAssembly == true)
				{
					...
					{
						if (OP.Configuration == Projects.ProjectOutputConfiguration.Debug && COF.Configuration == Projects.ProjectOutputConfiguration.Debug && IsConfigurationDebug(ProjectName) == true)
						{
							if (OP.Platform == Projects.ProjectOutputPlatform.AnyCPU && COF.Platform == Projects.ProjectOutputPlatform.AnyCPU && IsPlatformAnyCPU(ProjectName) == true) COF.Copy(OP.Path);
							if (OP.Platform == Projects.ProjectOutputPlatform.Itanium && COF.Platform == Projects.ProjectOutputPlatform.Itanium && IsPlatformItanium(ProjectName) == true) COF.Copy(OP.Path);
							if (OP.Platform == Projects.ProjectOutputPlatform.x64 && COF.Platform == Projects.ProjectOutputPlatform.x64 && IsPlatformX64(ProjectName) == true) COF.Copy(OP.Path);
							if (OP.Platform == Projects.ProjectOutputPlatform.x86 && COF.Platform == Projects.ProjectOutputPlatform.x86 && IsPlatformX86(ProjectName) == true) COF.Copy(OP.Path);
						}
						if (OP.Configuration == Projects.ProjectOutputConfiguration.Release && COF.Configuration == Projects.ProjectOutputConfiguration.Release && IsConfigurationRelease(ProjectName) == true)
						{
							if (OP.Platform == Projects.ProjectOutputPlatform.AnyCPU && COF.Platform == Projects.ProjectOutputPlatform.AnyCPU && IsPlatformAnyCPU(ProjectName) == true) COF.Copy(OP.Path);
							if (OP.Platform == Projects.ProjectOutputPlatform.Itanium && COF.Platform == Projects.ProjectOutputPlatform.Itanium && IsPlatformItanium(ProjectName) == true) COF.Copy(OP.Path);
							if (OP.Platform == Projects.ProjectOutputPlatform.x64 && COF.Platform == Projects.ProjectOutputPlatform.x64 && IsPlatformX64(ProjectName) == true) COF.Copy(OP.Path);
		...
		List<CompilerOutputFile> GetXSDWSDLCompilerOutput(string ProjectName, Projects.ProjectOutputFramework Framework)
		{
			...
			{
				COFL.Add(new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Framework), Path.GetFileName(f), true, Framework, CompilerOutputLanguage.None, CompilerOutputFileType.XSDWSDL)); ;
				COFL.Add(new CompilerOutputFile(this, GetBuildOutputPath(ProjectName, Framework), Path.GetFileName(f), false, Framework, CompilerOutputLanguage.None, CompilerOutputFileType.XSDWSDL)); ;
			}
	...
	internal class CompilerOutputFile
	{
		...
		private Projects.ProjectOutputPlatform platform;
		private Projects.ProjectOutputConfiguration configuration;
		...
		public Projects.ProjectOutputPlatform Platform { get { return platform; } }
		public Projects.ProjectOutputConfiguration Configuration { get { return configuration; } }
		public CompilerOutputLanguage Language { get { return language; } }

		public CompilerOutputFile(Compiler Compiler, string BuildPath, string FileName, bool IsServerFile, Projects.ProjectOutputFramework Framework, CompilerOutputLanguage Langauage = CompilerOutputLanguage.CSharp, CompilerOutputFileType FileType = CompilerOutputFileType.Code)
		{
			compiler = Compiler;
			buildPath = BuildPath;
			fileName = FileName;
		...
		public CompilerOutputFile(Compiler Compiler, string BuildPath, string FileName, bool IsServerFile, Projects.ProjectOutputFramework Framework, Projects.ProjectOutputPlatform Platform, Projects.ProjectOutputConfiguration Configuration, CompilerOutputLanguage Langauage = CompilerOutputLanguage.CSharp)
		{
			compiler = Compiler;
			buildPath = BuildPath;
			fileName = FileName;
			isServerFile = IsServerFile;
			isClientFile = !IsServerFile;
			fileType = CompilerOutputFileType.Assembly;
			framework = Framework;
			platform = Platform;
		...
		public string FileName
		{
			get
			{
				...
				if (FileType == CompilerOutputFileType.Assembly)
				{
					...
					FN.Append(".");
					FN.Append(System.Enum.GetName(typeof(Projects.ProjectOutputConfiguration), configuration));
					FN.Append(".");
					FN.Append(System.Enum.GetName(typeof(Projects.ProjectOutputPlatform), platform));
				}
				else
				{
			...
			if (FileType == CompilerOutputFileType.Assembly)
			{
				try
				{
					if (File.Exists(DestPath + FileName + ".dll"))
					{
						...
						{
							Compiler.Program.AddMessage(new CompileMessage("OS5000", "The destination file '" + FileName + ".dll' is marked as read-only. Please set the file to set the file as writeable and compile the project again.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output);
							return false;
						}
					}
				...
				catch (UnauthorizedAccessException) { Compiler.Program.AddMessage(new CompileMessage("OS5001", "Failed! You do not have permission to access the path '" + DestPath + "'.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output); return false; }
				catch (ArgumentException) { Compiler.Program.AddMessage(new CompileMessage("OS5002", "Failed! The path '" + DestPath + "' contains invalid characters.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output); return false; }
				catch (PathTooLongException) { Compiler.Program.AddMessage(new CompileMessage("OS5003", "Failed! The path '" + DestPath + "' is too long.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output); return false; }
				catch (DirectoryNotFoundException) { Compiler.Program.AddMessage(new CompileMessage("OS5004", "Failed! The path '" + DestPath + "' was not found.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output); return false; }
				catch (FileNotFoundException) { Compiler.Program.AddMessage(new CompileMessage("OS5005", "Failed! The source file was not found. This is typically due to a failed build.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output); return false; }
				catch (IOException) { Compiler.Program.AddMessage(new CompileMessage("OS5006", "Failed! I/O Error.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output); return false; }
				catch (NotSupportedException) { Compiler.Program.AddMessage(new CompileMessage("OS5007", "Failed! The path '" + DestPath + "' contains invalid characters.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output); return false; }
				catch (Exception) { Compiler.Program.AddMessage(new CompileMessage("OS5008", "Failed! Unspecified Reason.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output); return false; }
				try
				{
					if (File.Exists(DestPath + FileName + ".pdb"))
					{
						...
						if (fi.IsReadOnly == true)
						{
							Compiler.Program.AddMessage(new CompileMessage("OS5000", "The destination file '" + FileName + ".pdb' is marked as read-only. Please set the file to set the file as writeable and compile the project again.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output);
							return false;
						}
					}
				...
				if (language == CompilerOutputLanguage.CSharp)
				{
					try
					{
						if (File.Exists(DestPath + FileName + ".cs"))
						{
							...
							if (fi.IsReadOnly == true)
							{
								Compiler.Program.AddMessage(new CompileMessage("OS5000", "The destination file '" + FileName + ".cs' is marked as read-only. Please set the file to set the file as writeable and compile the project again.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output);
								return false;
							}
						}
				...
				else if (language == CompilerOutputLanguage.VisualBasic)
				{
					try
					{
						if (File.Exists(DestPath + FileName + ".vb"))
						{
							...
							if (fi.IsReadOnly == true)
							{
								Compiler.Program.AddMessage(new CompileMessage("OS5000", "The destination file '" + FileName + ".vb' is marked as read-only. Please set the file to set the file as writeable and compile the project again.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output);
								return false;
							}
						}
			...
			if (FileType == CompilerOutputFileType.XSDWSDL)
			{
				try
				{
					if (File.Exists(DestPath + FileName))
					{
						...
						if (fi.IsReadOnly == true)
						{
							Compiler.Program.AddMessage(new CompileMessage("OS5000", "The destination file '" + FileName + "' is marked as read-only. Please set the file to set the file as writeable and compile the project again.", CompileMessageSeverity.Error, null, null, null), CompilerMessageStage.Output);
							return false;
						}
					}
