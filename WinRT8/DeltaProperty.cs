...
namespace System
{
	...
	public sealed class DeltaProperty<T> : DeltaPropertyBase
	{
		...
		private DeltaProperty(HashID ID, HashID OwnerID, Type OwnerType, DependencyProperty XAMLProperty = null)
		{
			this.ID = ID;
		...
		private DeltaProperty(HashID ID, HashID OwnerID, Type OwnerType, T DefaultValue, DependencyProperty XAMLProperty = null)
		{
			this.ID = ID;
			this.OwnerID = OwnerID;
			this.OwnerType = OwnerType;
		...
		private DeltaProperty(HashID ID, HashID OwnerID, Type OwnerType, T DefaultValue, Action<DeltaObject, T, T> DeltaPropertyChangedCallback, Action<DeltaObject, T, T> DeltaPropertyUpdatedCallback = null, Func<DeltaObject, T, bool> DeltaValidateValueCallback = null, DependencyProperty XAMLProperty = null)
		{
			this.ID = ID;
			this.OwnerID = OwnerID;
			this.OwnerType = OwnerType;
		...
		public static DeltaProperty<TType> Register<TType>(string Name, Type OwnerType, DependencyProperty XAMLProperty = null)
		{
			var np = new DeltaProperty<TType>(HashID.GenerateHashID(OwnerType.FullName + "." + Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType, XAMLProperty);
		...
		public static DeltaProperty<TType> Register<TType>(string Name, Type OwnerType, TType defaultValue, DependencyProperty XAMLProperty = null)
		{
			var np = new DeltaProperty<TType>(HashID.GenerateHashID(OwnerType.FullName + "." + Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType, defaultValue, XAMLProperty);
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DeltaProperty '{0}' on type '{1}'. A DeltaProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}
		...
		{
			var np = new DeltaProperty<TType>(HashID.GenerateHashID(OwnerType.FullName + "." + Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType, defaultValue, DeltaPropertyChangedCallback, DeltaPropertyUpdatedCallback, DeltaValidateValueCallback, XAMLProperty);
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DeltaProperty '{0}' on type '{1}'. A DeltaProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}
