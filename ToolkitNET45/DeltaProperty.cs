...
namespace DeltaV
{
	...
	public sealed class DeltaProperty<T> : DeltaPropertyBase
	{
		...
		{
			var np = new DeltaProperty<TType>(HashID.GenerateHashID(Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType);
		...
		{
			var np = new DeltaProperty<TType>(HashID.GenerateHashID(Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType, defaultValue);
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DeltaProperty '{0}' on type '{1}'. A DeltaProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}
		...
		{
			var np = new DeltaProperty<TType>(HashID.GenerateHashID(Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType, defaultValue, DeltaPropertyChangedCallback);
		...
		{
			var np = new DeltaProperty<TType>(HashID.GenerateHashID(Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType, defaultValue, DeltaPropertyChangedCallback, DeltaValidateValueCallback);
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DeltaProperty '{0}' on type '{1}'. A DeltaProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}

		public static DeltaProperty<DeltaDictionary<TKey, TType>> RegisterDictionary<TKey, TType>(string Name, Type OwnerType)
		{
			var np = new DeltaProperty<DeltaDictionary<TKey, TType>>(HashID.GenerateHashID(Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType) {Mode = PropertyMode.Dictionary};
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DeltaProperty '{0}' on type '{1}'. A DeltaProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}

		public static DeltaProperty<DeltaList<TType>> RegisterList<TType>(string Name, Type OwnerType)
		{
			var np = new DeltaProperty<DeltaList<TType>>(HashID.GenerateHashID(Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType) {Mode = PropertyMode.List};
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DeltaProperty '{0}' on type '{1}'. A DeltaProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}

		public static DeltaProperty<DeltaQueue<TType>> RegisterQueue<TType>(string Name, Type OwnerType)
		{
			var np = new DeltaProperty<DeltaQueue<TType>>(HashID.GenerateHashID(Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType) {Mode = PropertyMode.Stack};
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DeltaProperty '{0}' on type '{1}'. A DeltaProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}

		public static DeltaProperty<DeltaQueue<TType>> RegisterStack<TType>(string Name, Type OwnerType)
		{
			var np = new DeltaProperty<DeltaQueue<TType>>(HashID.GenerateHashID(Name), HashID.GenerateHashID(OwnerType.FullName), OwnerType) {Mode = PropertyMode.Queue};
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DeltaProperty '{0}' on type '{1}'. A DeltaProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}
