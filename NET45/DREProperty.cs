...
namespace System
{
	public sealed class DREProperty<T> : CMDPropertyBase
	{
		...
		private DREProperty(HashID ID, string Name, HashID OwnerID, Type OwnerType, bool EnableBatching, DependencyProperty XAMLProperty = null, DependencyPropertyKey XAMLPropertyKey = null)
		{
			this.ID = ID;
			this.Name = Name;
		...
		private DREProperty(HashID ID, string Name, HashID OwnerID, Type OwnerType, bool EnableBatching, T DefaultValue, DependencyProperty XAMLProperty = null, DependencyPropertyKey XAMLPropertyKey = null)
		{
			this.ID = ID;
			this.Name = Name;
			this.OwnerID = OwnerID;
		...
		private DREProperty(HashID ID, string Name, HashID OwnerID, Type OwnerType, bool EnableBatching, Action<DREObjectBase, T, T> DREPropertyChangedCallback)
		{
			this.ID = ID;
			this.Name = Name;
			this.OwnerID = OwnerID;
		...
		{
			var np = new DREProperty<TType>(HashID.GenerateHashID(OwnerType.FullName + "." + Name), Name, HashID.GenerateHashID(OwnerType.FullName), OwnerType, EnableBatching, XAMLProperty, XAMLPropertyKey);
		...
		{
			var np = new DREProperty<TType>(HashID.GenerateHashID(OwnerType.FullName + "." + Name), Name, HashID.GenerateHashID(OwnerType.FullName), OwnerType, EnableBatching, DefaultValue, XAMLProperty, XAMLPropertyKey);
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DREProperty '{0}' on type '{1}'. A DREProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}
		...
		{
			var np = new DREProperty<TType>(HashID.GenerateHashID(OwnerType.FullName + "." + Name), Name, HashID.GenerateHashID(OwnerType.FullName), OwnerType, EnableBatching, DefaultValue, DREPropertyChangedCallback, DREPropertyUpdatedCallback, DeltaValidateValueCallback, XAMLProperty, XAMLPropertyKey);
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DREProperty '{0}' on type '{1}'. A DREProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}
		...
		{
			var np = new DREProperty<DeltaDictionary<TKey, TType>>(HashID.GenerateHashID(OwnerType.FullName + "." + Name), Name, HashID.GenerateHashID(OwnerType.FullName), OwnerType, true, DREPropertyChangedCallback) { IsDictionary = true };
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DREProperty '{0}' on type '{1}'. A DREProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}
		...
		{
			var np = new DREProperty<DeltaList<TType>>(HashID.GenerateHashID(OwnerType.FullName + "." + Name), Name, HashID.GenerateHashID(OwnerType.FullName), OwnerType, true, DREPropertyChangedCallback) { IsList = true };
			if (!registered.TryAdd(np.ID, np))
				throw new ArgumentException(string.Format("Unable to register the DREProperty '{0}' on type '{1}'. A DREProperty with the same Name and OwnerType has already been registered.", Name, np.OwnerType));
			return np;
		}
